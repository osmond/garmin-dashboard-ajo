...
function Switch({
  ...
  return (
    <SwitchPrimitive.Root
      data-slot="switch"
      className={cn(
        "peer data-[state=checked]:bg-[var(--primary)] data-[state=unchecked]:bg-[var(--input)] focus-visible:border-[var(--ring)] focus-visible:ring-[color:var(--ring)/.5] dark:data-[state=unchecked]:bg-[color:var(--input)/.8] inline-flex h-[1.15rem] w-8 shrink-0 items-center rounded-full border border-transparent shadow-xs transition-all outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    >
